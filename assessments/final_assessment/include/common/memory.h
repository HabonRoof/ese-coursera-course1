 ...
 *
 * @return a pointer of desitnation
 */
uint8_t* my_memmove(uint8_t* src, uint8_t * dst, size_t length);

/**
 * @brief my memory copy
 * 
 * This function takes two byte pointers (one source and one destination) 
 ...
 * @param src pointer to source array
 * @param length data length
 * @param value the vale to be set to array
 *
 * @return a pointer of source
 */
uint8_t* my_memset(uint8_t* src, size_t length, uint8_t value);

/**
 * @brief my memory set zero
 * 
...
/**
 * @brief reverse the words
 * 
 * This should take number of words to allocate in dynamic memory
 * 
 * @param length data length
 *
 * @return a pointer to memory if successful, Null Pointer if not
 */
